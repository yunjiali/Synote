			...
			<div>
				<div id="recording_list_div">
					...
					<g:each in="${multimediaList.rows}" var="row">
						<g:render template="/common/recording" model="['row':row,'editable':true,'actionEnabled':true, 'viewTranscriptsEnabled':false,'nerditEnabled':true, 'viewSynmarksEnabled':false]"/>
					</g:each>
				</div>
			</div>
